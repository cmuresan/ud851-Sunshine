...
//      COMPLETED (7) Generate a weather summary with the date, description, high and low
        ...
        double high = mCursor.getDouble(MainActivity.INDEX_COLUMN_MAX_TEMP);
        double low = mCursor.getDouble(MainActivity.INDEX_COLUMN_MIN_TEMP);

        String temperature = SunshineWeatherUtils.formatHighLows(mContext, high, low);

        String weatherSummary = dateString + " - " + description + " - " + temperature;
